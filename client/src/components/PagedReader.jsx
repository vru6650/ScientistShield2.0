...
export default function PagedReader({
  ...
  const [zoom, setZoom] = useState(() => {
    try { return Math.min(1.2, Math.max(0.6, parseFloat(localStorage.getItem('paged-zoom')) || 1)); } catch { return 1; }
  });
  ...
  const [bookMode, setBookMode] = useState(() => { try { return localStorage.getItem('paged-book') === '1'; } catch { return false; } });
  const [aspect, setAspect] = useState(() => { try { return localStorage.getItem('paged-aspect') || 'classic'; } catch { return 'classic'; } });
  const aspectRatio = aspect === 'a' ? 0.707 : aspect === 'golden' ? 0.618 : 0.66; // width/height
  const [showSurfaceTools, setShowSurfaceTools] = useState(() => { try { return localStorage.getItem('paged-tools') === '1'; } catch { return false; } });
  const [showSurfaceHighlights, setShowSurfaceHighlights] = useState(() => { try { return localStorage.getItem('paged-hl') === '1'; } catch { return false; } });
  const [activeSection, setActiveSection] = useState('');
  const [spread, setSpread] = useState(() => { try { return localStorage.getItem('paged-spread') === '1'; } catch { return false; } });
  ...
  const updatePages = useCallback(() => {
    ...
    try {
      ...
      const fracValue = size === 's' ? 0.65 : size === 'l' ? 0.85 : (pageHeight || 0.75);
  ...
  }, [autoHeight, vh, size, zoom, aspectRatio, bookMode, pageHeight, spread]);
    ...
    const onResize = () => {
      ...
      try {
        const el = containerRef.current; if (el) {
          ...
          const avail = Math.max(320, Math.floor(window.innerHeight - rect.top - 24));
          const frac = Math.floor((window.innerHeight) * sizeToFrac * zoom);
          setAutoHeight(Math.min(avail, frac));
        }
      } catch (_) {}
      updatePages();
    };
  ...
  const style = { '--page-height': `${autoHeight ?? Math.round((vh || 0) * sizeToFrac * zoom)}px` };
  ...
  const changeZoom = (op) => {
    ...
    setZoom(next);
    try { localStorage.setItem('paged-zoom', String(next)); } catch {}
    requestAnimationFrame(() => {
      const el = containerRef.current; if (el) {
        const rect = el.getBoundingClientRect();
        const avail = Math.max(320, Math.floor(window.innerHeight - rect.top - 24));
        const frac = Math.floor((window.innerHeight) * sizeToFrac * next);
        setAutoHeight(Math.min(avail, frac));
      }
      updatePages();
    });
  ...
  const toggleSurfaceTools = () => {
    setShowSurfaceTools((v) => { const next = !v; try { localStorage.setItem('paged-tools', next ? '1' : '0'); } catch {} return next; });
  };
  const toggleSurfaceHighlights = () => {
    setShowSurfaceHighlights((v) => { const next = !v; try { localStorage.setItem('paged-hl', next ? '1' : '0'); } catch {} return next; });
  };
  ...
  // Recompute layout when key reading settings or size changes
  useEffect(() => {
    const id = requestAnimationFrame(() => {
      ...
      try {
        const rect = el.getBoundingClientRect();
        const avail = Math.max(320, Math.floor(window.innerHeight - rect.top - 24));
        const frac = Math.floor((window.innerHeight) * sizeToFrac);
        setAutoHeight(Math.min(avail, frac));
      } catch (_) {}
      updatePages();
    });
    return () => cancelAnimationFrame(id);
  }, [size, zoom, content, settings?.fontSize, settings?.lineHeight, contentMaxWidth, sizeToFrac, updatePages]);

  const toggleBookMode = () => {
    setBookMode((v) => {
      const next = !v; try { localStorage.setItem('paged-book', next ? '1' : '0'); } catch {}
      requestAnimationFrame(updatePages);
      return next;
    });
  };
  ...
  const cycleAspect = () => {
    ...
    setAspect(next);
    try { localStorage.setItem('paged-aspect', next); } catch {}
    requestAnimationFrame(updatePages);
  };
  ...
  return (
    <div className={`paged-reader ${bookMode ? 'book-mode' : ''} ${spread ? 'spread' : ''} ${idle ? 'reader-idle' : ''} ${isTurning ? `tilt-${isTurning}` : ''} ${className}`} style={style}>
      {showReadingControls && (
      ...
      <div className="paged-toolbar">
        ...
        <div className="paged-indicator" aria-live="polite">
          Page {page + 1} / {pages}
        ...
        <div className="paged-zoom">
          <button type="button" className="paged-size-btn" onClick={() => changeZoom('dec')} aria-label="Zoom out">−</button>
          <span className="paged-zoom-label">{Math.round(zoom * 100)}%</span>
          <button type="button" className="paged-size-btn" onClick={() => changeZoom('inc')} aria-label="Zoom in">＋</button>
        </div>
        <div className="paged-book">
          <button type="button" className={`paged-size-btn ${bookMode ? 'active' : ''}`} onClick={toggleBookMode} aria-label="Toggle book page">Book</button>
          {bookMode && (
            <button type="button" className="paged-size-btn" onClick={cycleAspect} aria-label="Cycle aspect">
              {aspect === 'classic' ? '2:3' : aspect === 'a' ? 'A' : 'φ'}
            </button>
          )}
        </div>
        <div className="paged-actions">
          <button type="button" className={`paged-size-btn ${showSurfaceTools ? 'active' : ''}`} onClick={toggleSurfaceTools} aria-label="Toggle reading tools">Tools</button>
          <button type="button" className={`paged-size-btn ${showSurfaceHighlights ? 'active' : ''}`} onClick={toggleSurfaceHighlights} aria-label="Toggle highlights panel">HL</button>
          <button type="button" className="paged-size-btn" onClick={cycleMargin} aria-label="Cycle margin">MG</button>
          <button type="button" className={`paged-size-btn ${spread ? 'active' : ''}`} onClick={() => { const next = !spread; setSpread(next); try { localStorage.setItem('paged-spread', next ? '1' : '0'); } catch {}; requestAnimationFrame(updatePages); }} aria-label="Toggle two-page spread">Spread</button>
          <button type="button" className="paged-size-btn" onClick={() => setShowHelp((s) => !s)} aria-label="Shortcuts help">?</button>
        </div>
        {headings.length > 0 && (
          <div className="paged-actions">
